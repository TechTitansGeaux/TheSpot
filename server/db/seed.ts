...
const seedSqlize = () => {
  mysql.createConnection({ user: 'root', password: '', host: '127.0.0.1' })
    ...
    .then(() => Promise.all(require('./fakeUserData.ts').map((txn) => Users.create(txn))))
    .then(() => Promise.all(require('./fakePlaceData.ts').map((txn) => Places.create(txn))))
    .then(() => Promise.all(require('./fakeEventData.ts').map((txn) => Events.create(txn))))
    .then(() => Promise.all(require('./fakeData.ts').map((txn) => Reels.create(txn))))
    .then(process.exit);
};
